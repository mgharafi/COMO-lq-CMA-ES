    ...
    def surrogate_tell(self, solutions, objective_values, surrogate_uhvi = None):
        ...
        if self.NDA is None:
            ...
            self.NDA = BiobjectiveNondominatedSortedList if len(
                    objective_values[last_key]) == 2 else NonDominatedList
        ...
        for ikernel, offspring in self.offspring:
            ...
            objective_vals = [objective_values.get(off) for off in offspring if not objective_values.get(off, None) is None]
...
class LQCOMO:
    ...
    def __init__(self, X0, sigma0, fitness, reference_point,
        inopts={}, use_surrogate = True, output="exdata",
        tau_tresh = .85, log=False, min_evals_percent=0, return_true_fitnesses=True,
        ...
        for ikernel, kernel in enumerate(self.solver):
            ...
            if self.log:
                ...
                kernel.surrogate.logger.push_, kernel.surrogate.logger.push = kernel.surrogate.logger.push, lambda : None
    ...
    def uhvi(self, kernel, forget=False):
        ...
        def wrapper(x):
            ...
            # Return -uhvi(x)
            return -float(self.solver.indicator_front.hypervolume_improvement(self.tracer(x, forget=forget)))

        return wrapper

    def optimize(self, n_ask=1, user_stop=lambda:None, resetModel=False, user_updates_rules='default', use_incumbents=True, callback=None, injectModelOptimum=True):
        ...
        while not self.solver.stop() and not user_stop():
            ...
            if self.solver.surrogate: # Tell the surrogate case
                ...
                for ikernel, off in self.solver.offspring: # Evaluate the offsprings on the surrogate
                    """
                    ...
                    if user_updates_rules(kernel):
                        ...
                        if len(mX):  # When the archive is not empty update UHVI values
                            ...
                            kernel.surrogate.model._coefficients_count = kernel.surrogate.model.count - 1
    ...
    def plotSurrogate(self, kernel, ax, drawCurves=['ttau0', 'tau0', 'tau1', 'neval', 'ttau1']):
        ...
        styles = [{'lw':lw}, {'ls':'--', 'lw':lw}, {'ls':'--', 'lw':lw}, {'c':'red', 'lw':lw}, {'lw':lw}]
        ...
        ax.axhline(y=surrogate.settings.tau_truth_threshold, ls=":", alpha=.4, c='black', lw=lw)
